...
This capsule is the per-brain **worker** stage of a larger Code Ocean pipeline. In the original analysis it was driven by a separate *dispatcher* capsule, which writes the `dispatched_zarr*.json` task files this capsule consumes and a *pipeline* capsule that ran the worker across all 8 brains in parallel. Those orchestration capsules are not included here and the dispatched task files are produced upstream, so **this released version does not run reproducibly on its own**, and the data assets published with the paper were not generated by this exact released code.

It is shared for transparency: the segmentation and point-detection code behind the published results can be read and inspected here, and the inputs it operates on are publicly available. The 8 whole-brain SmartSPIM volumes in `s3://aind-open-data` and the `FOS_model_210815` UNet weights so the steps can be followed even though the original run is not reproduced exactly.
